class IosDeployPlatform < Formula
  ...
  url "https://github.com/snooky23/ios-deploy-platform/archive/refs/tags/v2.8.0.tar.gz"
  license "MIT"
  version "2.8.0"
  sha256 "ac3d58414207d7a108463b4d8dce89cf66ca59510302e1c2eb4d5d3a9647b622"
  ...
  def install
    ...
    config_example_file = etc/"ios-deploy/config.example"
  ...
  def ios_deploy_script
    <<~EOS
      ...
      show_usage() {
          cat <<EOF
      📱 iOS FastLane Auto Deploy v2.8.0
      ...
      main() {
          case "$1" in
              "help"|"--help"|"-h"|"")
                  ...
                  ;;
              "version"|"--version"|"-v")
                  echo "iOS FastLane Auto Deploy v2.8.0"
  ...
  def man_page_content
    <<~EOS
      .TH IOS-DEPLOY 1 "January 2025" "ios-deploy 2.8.0" "iOS Development Tools"
      .SH NAME
  ...
  test do
    ...
    assert_match "iOS FastLane Auto Deploy v2.8.0", shell_output("#{bin}/ios-deploy version")
